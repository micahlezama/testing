...
def train_team(locui: list[int], loci: list[int], ucards: list, custom_levels=[], skip_awakened=False):
    try:
        ...
        print(Fore.RED + "\n[Train] ❌ Training interrupted — returning to main menu..." + Style.RESET_ALL)
        return 0
...
def run():
    try:
        ...
        for char in card_list:
            ...
            if char['Hercule'] != 1 and char['HP'] > 5:
                cards_to_display.append(
                    f"{char['UniqueID']} | {char['ID']} | {char['Name']} | {char['Level']}"
                )
        ...
        print(Back.LIGHTCYAN_EX + (Fore.GREEN if notc > 0 else Fore.RED) +
              f'Trained {notc}/{len(chosen_unique_ids)} cards!')

        return 0

    except KeyboardInterrupt:
        print(Fore.RED + "\n[Train] ❌ Cancelled — returning to main menu..." + Style.RESET_ALL)
        return 0
